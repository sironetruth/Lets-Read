export default function Mascot({ size = 120, mood = 'happy', className = '', accessory = null }) {
  // Kiwi the reading parrot — the app's signature character.
  // mood: 'happy' (gentle idle bob) | 'excited' (bigger bounce + wiggle, used for greetings/celebrations)
  // accessory: optional emoji rendered as a small overlay (unlockable dress-up)
  return (
    <span className="mascot-wrap" style={{ width: size, height: size }}>
      ...
      {accessory && (
        <span className="mascot-accessory" style={{ fontSize: size * 0.36 }} aria-hidden="true">
          {accessory}
        </span>
      )}
    </span>
